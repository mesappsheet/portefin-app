...
- **Synchronisation Bidirectionnelle & Bouton de Synchronisation Globale des Tâches (PWA & Google Tasks)** : Synchronisation bidirectionnelle de l'état de complétion (terminée ou non) des tâches. Le fait de cocher ou décocher une tâche dans la PWA appelle en tâche de fond `syncTaskStatusToGoogleMobile(task)` via l'action Google Apps Script `toggle_task_status` pour mettre à jour Google Tasks. Un bouton de synchronisation globale `cloud_sync` a été ajouté dans la barre d'outils des tâches pour lancer `syncAllTasksMobile()`, qui récupère toutes les tâches de Google Tasks (via `list_tasks`), synchronise bidirectionnellement leur statut avec Supabase, et exporte sur Google Tasks (et Google Agenda) les tâches Supabase manquantes. Implémenté le 2026-06-20.
