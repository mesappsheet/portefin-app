...
- **Sélection multiple (appui long) & Actions en un clic sans confirmation pour l'état d'une tâche (PWA)** : Un appui long (600ms) sur n'importe quelle carte de tâche (terminée ou non terminée) active le mode de sélection multiple pour suppression en masse. Dans ce mode, un bouton de sélection rond (checkbox) apparaît à la place du bouton de statut devant chaque tâche, les cartes sélectionnées s'illuminent en orange (`#f27f0d`) avec un fond teinté, et une barre flottante premium (glassmorphic) apparaît en bas montrant le nombre de tâches sélectionnées avec options "Tout sélectionner/Désélectionner tout" et "Supprimer" (confirmation groupée unique pour l'appel d'API `delete().in('id', ...)`). De plus, lorsque le mode sélection est inactif, le changement de statut via le bouton rond de statut devant la tâche s'exécute en un seul clic sans aucune invite de confirmation. Implémenté le 2026-06-21.
